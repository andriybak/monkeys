...
def register():
    form = RegisterForm()
    if form.validate_on_submit():
        newuser = User(nickname=form.nickname.data,
...
@app.route("/all_users/o<int:order>")
@login_required
def all_users(order=1, page=1):
    if order == 1:
        allusers = g.user.users_by_name().paginate(page, POSTS_PER_PAGE, False)
    elif order == 2:
        allusers = g.user.users_by_bff().paginate(page, POSTS_PER_PAGE, False)
#    allusers=User.query.paginate(page,POSTS_PER_PAGE,False).order_by(User.nickname.desc())
